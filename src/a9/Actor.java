...
public class Actor extends Sprite implements Attack {
	...
	// Every Actor now has a timer that updates every half second it is alive. This helps with abilities as well as despawning the Chi.
	private double despawnTime;
	private Timer despawn = new Timer();
	private TimerTask task = new TimerTask() {
		public void run() {
			// Every iteration, increase the seconds counter.
			despawnTime = despawnTime + 0.5;
		}
	};
	...
	public Actor(Point2D.Double startingPosition, Point2D.Double initHitbox, BufferedImage img, int health, int coolDown, double speed, int attackDamage) {
		...
		// Start the timer, iterating every 500 milliseconds.
		despawn.scheduleAtFixedRate(task, 500, 500);
	}

	/**
	 * Getter for the despawnTime variable. Which counts the 
	 * seconds an Actor is alive.
	 * 
	 * @return despawnTime seconds living
	 */
